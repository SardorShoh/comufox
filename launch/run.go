...
func setEnvironmentVariables(configMap map[string]interface{}, userAgentOS string) {
	...
	var chunkSize int
	if dirs.NormalizeOS(runtime.GOOS) == "windows" {
		chunkSize = 2047
	} else {
		chunkSize = 32767
	}
	...
	if dirs.NormalizeOS(runtime.GOOS) == "linux" {
